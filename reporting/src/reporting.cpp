...
ACTION reporting::init() {
		...
		users.emplace(_self, [&]( auto& row ) { row.user = "oone"_n; row.balance = 10; row.statusR = 10; row.statusV = 0; row.blames = 0; row.verificator = 1; row.frozen = 0; });
		users.emplace(_self, [&]( auto& row ) { row.user = "delekom"_n; row.balance = 10;row.statusR = 10; row.statusV = 0; row.blames = 0; row.verificator = 1; row.frozen = 0; });
		users.emplace(_self, [&]( auto& row ) { row.user = "bzz"_n; row.balance = 10; row.statusR = 7; row.statusV = 0; row.blames = 0; row.verificator = 0; row.frozen = 0; });
...
ACTION reporting::enrol(name user) {
  ...
  user_t users( _self, _self.value );
	...
	users.emplace(_self, [&]( auto& row ) { row.user = user; row.balance = 0; row.statusR = 0; row.statusV = 0; row.blames = 0; row.verificator = 0; row.frozen = 0; });
}
...
ACTION reporting::vote(uint64_t itemKey, name voter, uint64_t merit) {
	...
	if (!isVoteable && it_item->confirmations < minConfirmations) { item.modify(it_item, _self, [&]( auto& row ) { row.rating = 0; }); }
	//appoint as verificator if enough status points are earned
	if(it_reporter->statusR >= statusThreshold) { reporting::appoint( it_reporter->user); }
}
...
ACTION reporting::transfer(name from, name to, uint64_t amount) {
  ...
  check( !(it_to->frozen) , "The receiving account is frozen." );
	check( it_from->balance - amount < it_from->balance , "IntegerOverflow (sender).");
...
ACTION reporting::buy( name buyer, uint64_t itemKey ) {
	...
	for(auto& row : order) { check( !(row.itemKey == itemKey && row.buyer == buyer), "That order was already placed." ); }
	order.emplace(_self, [&]( auto& row ) { row.key = order.available_primary_key();  row.itemKey = itemKey; row.buyer = buyer; row.received = 0; });
	reporting::transfer(buyer, seller, price);
}

ACTION reporting::received( name buyer, uint64_t itemKey, bool done ) {
	require_auth( buyer );
	user_t users( _self, _self.value );
	auto it_buyer = users.find(buyer.value);
	check( !(it_buyer == users.end()) , "Please enrol first." );
	...
	order_t order( _self, _self.value );
	auto it_order = order.find(itemKey);
	check( !(it_order == order.end()), "No such order." );
	check( !(it_order->buyer == buyer ), "That's not your order." );
	if (done) { order.modify(it_order, _self, [&]( auto& row ) { row.received = 1; }); }
	else { 
	  item_t item( _self, _self.value );
	  auto it_item = item.find(it_order->itemKey);
	  reporting::blame(buyer, it_item->reporter, "Order " + it_order->itemKey + " not received.", 1);
	}
}
...
ACTION reporting::blame(name blamer, name blamed, std::string reason, bool freeze) {
	...
	blaming.emplace( _self, [&]( auto& row ) { row.key = blaming.available_primary_key(); row.blamer = blamer;  row.blamed = blamed; row.reason = reason; row.freeze = freeze; row.voteable = 1; row.confirmations = 0; row.votes = 0; });
}
...
ACTION reporting::voteb(uint64_t blameKey, name voter, bool value) {
	...
	for(auto& row : votingb) { check( !( row.blameKey == blameKey && row.voter == voter ), "You already voted for that item." ); }	
	
	bool isVoteable = 1; if (it_blaming->votes + 1 >= voteThreshold) { isVoteable = 0; }
	votingb.emplace(_self, [&]( auto& row ) { row.key = votingb.available_primary_key(); row.blameKey = blameKey; row.voter = voter; row.value = value; });
	blaming.modify(it_blaming, _self, [&]( auto& row ) { row.confirmations = row.confirmations + value; row.votes = row.votes + 1; row.voteable = isVoteable; });
	if (!isVoteable && it_blaming->confirmations >= minConfirmationsBlame) { 
    auto it_blamed = users.find(it_blaming->blamed.value);
  	users.modify(it_blamed, _self, [&]( auto& row ) { row.blames = row.blames + it_blaming->freeze; });
  	if (it_blamed->blames >= blameThreshold) { users.modify(it_blamed, _self, [&]( auto& row ) { row.frozen = it_blaming->freeze; }); }
	}
}
